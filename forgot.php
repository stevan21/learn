...
if (isset($_POST['forgot'])) {
	...
	$_SESSION['email'] = $mail;
	if (!isset($mail)) {
		$err_mail = "Veuillez remplir ce champs!";
		$valide = false;
	}
	if ($valide) {
		$req_search_mail = "SELECT * FROM student WHERE mail = '".$mail."'";
		$query_search_mail = mysqli_query($edule, $req_search_mail);
		$forgot_mail = mysqli_fetch_assoc($query_search_mail);

		function idCode($longueur = 5) {
		...
		$code_nouveau = idCode();
		$_SESSION['code'] = $code_nouveau;

		$req_base_code = "UPDATE `student` SET `code` = '".$_SESSION['code']."' WHERE mail = '".$mail."'";
		$query_base_code = mysqli_query($edule, $req_base_code);

		$from = "stevanmultiservices@gmail.com";
		$to = $mail;
		$objet = "Réunitialisation du mot de passe";
		$message = "Votre code ";
		$message = $message." ".$code_nouveau;
		$headers = "From: stevan-Educ";
                            ...
                            <div class="register-login-form">
                                ...
                                <div class="form-wrapper">
                                    <form action="" method="post">
                                        ...
                                        <!-- Single Form Start -->
                                        <div class="single-form">
                                            <button name="forgot" class="btn btn-primary btn-hover-dark w-100">Envoyez</button>
                                        </div>
                                        <!-- Single Form End -->
                                    </form>
                                </div>
