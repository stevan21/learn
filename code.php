...
if (isset($_POST['code'])) {
	...
	$code = $_POST['code'];
	if (!isset($code)) {
		$err_code = "Veuillez remplir ce champs!";
		$valide = false;
	}
	if ($valide) {
		$req_search_code = "SELECT * FROM student WHERE mail = '".$mail."'";
		$query_search_code = mysqli_query($edule, $req_search_code);
		$forgot_code = mysqli_fetch_assoc($query_search_code);

        if ($forgot_code['code'] == $_SESSION['code']) {
            $_SESSION['id'] = $forgot_code['id'];
            header("Location: password.php");
        }else{
            $err_code_2 = "Code incorrect <br>";
        }
	}
}
 ?>
                            ...
                            <div class="register-login-form">
                                ...
                                <div class="form-wrapper">
                                    <form action="" method="post">
                                        ...
                                        <!-- Single Form Start -->
                                        <div class="single-form">
                                            <button name="code" class="btn btn-primary btn-hover-dark w-100">Envoyez</button>
                                        </div>
                                        <!-- Single Form End -->
                                    </form>
                                </div>
